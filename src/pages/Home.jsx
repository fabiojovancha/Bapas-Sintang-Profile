...
import {
  ...
  Users, FileText, Award, Shield,
  ClipboardList, Baby, Gavel, Eye, HelpCircle
} from 'lucide-react'
import PageWrapper from '@/components/UI/PageWrapper'
import BeritaCard  from '@/components/UI/BeritaCard'
import LayananCard from '@/components/UI/LayananCard'
import { LoadingSpinner } from '@/components/UI/StateBox'
import { useGAS }                      from '@/hooks/useGAS'
import { fetchBerita, fetchStatistik } from '@/services/api'
import { layananList }                 from '@/data/layanan'

const IKON_MAP = {
  Users, FileText, Award, Shield,
  ClipboardList, Baby, Gavel, Eye, Phone,
}

export default function Home() {
  const { data: beritaList,    loading: loadingBerita } = useGAS(fetchBerita)
  const { data: statistikList, loading: loadingStat   } = useGAS(fetchStatistik)
    ...
    <PageWrapper title="Beranda">
        ...
        <div className="container-bapas relative z-10 py-20 lg:py-28">
          <div className="max-w-3xl">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }}
              ...
              Balai Pemasyarakatan<br /><span className="text-gold-400">Kelas II Sintang</span>
            </motion.h1>
      ...
      <section className="bg-white border-b border-slate-100">
        <div className="container-bapas py-10">
          {loadingStat ? (
            <LoadingSpinner teks="Memuat statistik..." />
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              {(statistikList || []).map(({ label, value, ikon }, i) => {
                const Ikon = IKON_MAP[ikon] || HelpCircle
                return (
        ...
        <div className="container-bapas">
          <div className="text-center mb-12">
            ...
            <p className="section-subtitle mx-auto text-center">
              Bapas Kelas II Sintang menyediakan berbagai layanan bimbingan kemasyarakatan yang profesional.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {layananList.slice(0, 4).map((layanan, i) => (
              <motion.div key={layanan.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
          ...
          <div className="text-center mt-10">
            <Link to="/layanan" className="btn-outline">Lihat Semua Layanan <ChevronRight className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-white">
        <div className="container-bapas">
          ...
          {loadingBerita && <LoadingSpinner teks="Memuat berita terbaru..." />}
          {!loadingBerita && beritaList && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {beritaList.slice(0, 3).map((berita, i) => (
                <motion.div key={berita.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
          ...
          <div className="text-center mt-8 md:hidden">
            <Link to="/berita" className="btn-outline">Semua Berita <ArrowRight className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-primary-800">
        <div className="container-bapas text-center">
          <h2 className="text-3xl font-heading font-bold text-white mb-4">Butuh Informasi atau Bantuan?</h2>
          <p className="text-primary-300 mb-8 max-w-xl mx-auto">
            Tim kami siap membantu Anda. Hubungi kami melalui telepon, email, atau kunjungi kantor kami langsung.
          </p>
          <Link to="/kontak" className="btn-primary bg-gold-500 hover:bg-gold-400 text-primary-900">
            Hubungi Kami Sekarang <Phone className="w-4 h-4" />
          </Link>
        </div>
      </section>
